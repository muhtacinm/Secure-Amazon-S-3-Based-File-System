...
int e_or_d(int fd, int type){
    ...
    if(read(fd, input_b, offset)  == -1 ) { //read into input buffer
        	perror("READ ERROR");
        	exit(-1);
    }
    
    
    if(ftruncate(fd, 0) < 0){ //reduce file size to 0
      perror("FTRUNCATE ERROR");
      exit(-1);
    }
    
    lseek(fd, 0, SEEK_END); //reset buffer
    if (write(fd, "Salted__", 8) < 0){ //add phrase to begining of file
        perror("Error opening input file");
        return 1;
    }
    
    lseek(fd, 8, SEEK_SET);  
    if (write(fd, salt, 8) < 0){ //write random 8 bytes of salt
        perror("Error opening input file");
        return 1;
    }
    ...
    RC4(&key, offset, input_b, output_b); //perform encryption on input buffer to output buffer
    
    //write after first 16 bytes
    ...
    //read in file contents after first 16 bytes to input buffer
    lseek(fd, 16, SEEK_SET);
    ...
    //generate key
     if(!EVP_BytesToKey(cipher, dgst, (const unsigned char*)salt, (unsigned char *)eKey, eKeyLength, 1, rKey, NULL)) //salt key
     {
            fprintf(stderr, "EVP_BytesToKey failed\n");
            return 1;
     }
...
int FdEntity::RowFlush(const char* tpath, bool force_sync)
{
    ...
    S3FS_PRN_INFO3("[tpath=%s][path=%s][fd=%d] downloading ", SAFESTRPTR(tpath), tmppath.c_str(), fd);
